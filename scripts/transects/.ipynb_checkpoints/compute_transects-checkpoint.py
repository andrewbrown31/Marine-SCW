...
def plot_transects(path,f,xlim,ylim,no_of_transects=30,d_km=50):
    ...
    line_wdir = 270 - wdir.rolling({"south_north":11,"west_east":11}).mean().values[line_inds][0:-1:step]
    ...
    uv_ds = xr.Dataset({"u":u10,"v":v10}).coarsen(dim={"south_north":12,"west_east":12},boundary="trim").mean()
...
def get_transects(path,f,xlim,ylim,no_of_transects=30,d_km=50,top=12000,mean_motion_top=6000):
...
if __name__ == "__main__":
    ...
    xlims = [[152.0,154.6],[153.5,154.6],[153.5,154.6],[153.5,154.6]]
